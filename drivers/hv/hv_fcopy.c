...
char fcopy_devname[] = "vmbus/hv_fcopy";
u8 *recv_buffer;
...
void fcopy_timeout_func(struct work_struct *dummy)
{
...
int fcopy_handle_handshake(u32 version)
{
	return fcopy_handle_handshake_rust(version);
...
void fcopy_send_data(struct work_struct *dummy)
{
	fcopy_send_data_rust(dummy);
...
{
	fcopy_respond_to_host_rust(error);
...
{
	hv_fcopy_onchannelcallback_rust(context);
...
int fcopy_on_msg(void *msg, int len)
{
	return fcopy_on_msg_rust(msg, len);
	// int *val = (int *)msg;
...
{
	//not fully working
	return hv_fcopy_pre_suspend_rust();
